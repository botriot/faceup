...
function cvDetect(im, name, callback) {
  im.detectObject('./node_modules/opencv/data/'+name+'.xml', {min: [10, 10]}, callback)
}
      ...
      cv.readImage(image, function(err, im) {
        ...
        cvDetect(im, 'haarcascade_frontalface_alt2', function(err, faces) {
          ...
          async.forEach(faces, function(face, callback) {
            var face2 = cut(face, {x: -0.1, y: -0.1, width: 1.1, height: 1.1})
            async.parallel(
              {
                eyeglasses: async.apply(cvDetectSub, im, cut(face2, {height: 0.5}), 'haarcascade_eye_tree_eyeglasses'),
                eyepair: async.apply(cvDetectSub, im, cut(face2, {height: 0.6}), 'haarcascade_mcs_eyepair_small'),
                eye_left: async.apply(cvDetectSub, im, cut(face2, {width: 0.6}), 'haarcascade_righteye_2splits'),
                eye_right: async.apply(cvDetectSub, im, cut(face2, {x: 0.4, width: 0.6}), 'haarcascade_lefteye_2splits'),
                mouth: async.apply(cvDetectSub, im, cut(face2, {x: 0.3, width: 0.4, y:0.5, height: 0.5}), 'haarcascade_mcs_mouth'),
                nose: async.apply(cvDetectSub, im, cut(face2, {x: 0.2, width: 0.6, y: 0.2, height: 0.6}), 'haarcascade_mcs_nose'),
              },
