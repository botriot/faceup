...
function cvDetect(im, name, callback) {
  im.detectObject('./node_modules/opencv/data/'+name+'.xml', {}, callback)
}
      ...
      cv.readImage(image, function(err, im) {
        ...
        cvDetect(im, 'haarcascade_frontalface_alt2', function(err, faces) {
          ...
          async.forEach(faces, function(face, callback) {
            async.parallel(
              {
                eye_left: async.apply(cvDetect, im, 'haarcascade_mcs_righteye'),
                eye_right: async.apply(cvDetect, im, 'haarcascade_mcs_lefteye'),
                mouth: async.apply(cvDetect, im, 'haarcascade_mcs_mouth'),
                nose: async.apply(cvDetect, im, 'haarcascade_mcs_nose'),
              },
              function(err, results) {
                ...
                tags.push({
                  ...
                  mouth_center: center(results.mouth[0]),
                  nose: center(results.nose[0]),
                })
